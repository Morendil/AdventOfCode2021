...
solveStep scanners (solution, pairs) = (newSolution, remainingPairs)
    where candidatePairs = filter (\(a,b) -> isJust $ M.lookup a solution) pairs
          remainingPairs = filter (\(a,b) -> isNothing $ M.lookup b newSolution) pairs
          dists = map (\n -> distances $ scanners !! n) (nub $ map fst candidatePairs)
          ...
          solvePair solution (from, to) = if isNothing found then solution else solution'
              where found = match (dists !! from) (scanners !! to)
                    (_, origin) = fromJust $ M.lookup from solution
                    solution' = M.insert to solved solution
                    (o, p) = fromJust found
                    solved = (transform (scanners !! to) o, add p origin)
...
main = do
    ...
    let (solution,_) = last $ takeUntil solved $ iterate (solveStep scanners) (initial scanners, allPairs [0..length scanners-1])
